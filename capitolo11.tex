...
Il sistema è stato ri-progettato in modo da abbracciare il paradigma "all-IP", dunque il piano di controllo e il piano utente sono basati su IP e la commutazione di circuito non è più supportata.
...
\begin{itemize}
	...
	\item \textbf{Serving Gateway (SGW)}: instrada e inoltra i pacchetti dati degli utenti.
...
\begin{itemize}
	...
	\begin{enumerate}
		\item Il MME richiede a tutti gli eNodeB l'ultima Tracking Area nota per iniziare la procedura di paging.
...
A questo scopo, una nuova interfaccia (SG interface) è stata definita tra MME e MSC. Questa interfaccia facilita il fallback per chiamate vocali.
